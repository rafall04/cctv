...
| Mode Monitor v3 — opt-in per area (`areas.monitor_enabled`, admin toggle + checkbox) — picker/`?area=` hanya area yang diaktifkan; migrasi seed DS DANDER+TANJUNGHARJO+KEC BOJONEGORO | 2026-09-28 | 3 test backend `areaMonitorEnabled`, 19 test MonitorPage, 30/30 area-suite, 246/246 e2e; AreaManagement dipecah → `useAreaFlagToggles.js` (766 ln, keluar dari FROZEN) |
| CLS 0.184→0.00 di `?mode=full&view=map` — dua akar: (a) boot shell in-flow digeser swap mount → `position:fixed` overlay + `scrollbar-gutter:stable`; (b) grid hero `featured ? lg:grid-cols` baru restruktur saat payload kamera tiba ~800ms → slot spotlight dicadangkan skeleton segeometri kartu asli sejak paint pertama (`spotlight-skeleton`), ditarik saat data selesai tanpa kamera layak | 2026-09-28 | v1.4.159+160; trace live chrome-devtools: CLS 0.00 ×2 desktop, 0.02 mobile-throttled, LCP 722ms; 7/7 LandingHero test |
