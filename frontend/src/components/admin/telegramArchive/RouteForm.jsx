/*
 * Purpose: Add/edit fields for one Telegram archive route — scope, target camera or area, group id
 *          (with a pre-save check against Telegram), label, and enabled flag.
 * Caller: pages/TelegramArchiveSettings.jsx, inside a ui/Modal.
 * Deps: React, archiveUi class recipes.
 * MainFuncs: RouteForm, ROUTE_FORM_ID.
 *
 ...
 *
 * This renders the <form> and nothing around it: the heading, the Batal/Simpan row and the shell
 * belong to the dialog now. The submit button lives in the dialog's pinned footer, OUTSIDE this
 * element, and reaches back in through `form={ROUTE_FORM_ID}` — hence the exported id.
 */

import { btnGhost, hint, input, label } from './archiveUi';

export const ROUTE_FORM_ID = 'tga-route-form';
...
export function RouteForm({
    draft, setField, cameras, areas, groups, manual, onToggleManual, onPickGroup,
    onSubmit, onVerify, verifying, verified,
}) {
    const usePicker = groups.length > 0 && !manual;

    return (
        <form id={ROUTE_FORM_ID} onSubmit={onSubmit} className="space-y-5">
