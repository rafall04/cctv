        ...
        <form onSubmit={onSubmit} className="rounded-lg border border-edge bg-surface p-5 shadow-sm">
            <div className="grid gap-4 md:grid-cols-2">
                <label className="block min-w-0">
                    ...
                    <input value={form.label} onChange={(event) => onUpdateForm('label', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
                </label>
                <label className="block min-w-0">
                    <span className="mb-1 block text-sm font-medium text-content-muted">Preset (isi cepat)</span>
                    <select value={form.preset} onChange={(event) => onPresetChange(event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white">
                ...
                <label className="block min-w-0">
                    ...
                    <select value={form.scope_type} onChange={(event) => onUpdateForm('scope_type', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white">
                ...
                <div className="grid min-w-0 gap-3 sm:grid-cols-2">
                        ...
                        {isRange ? (
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="block w-full min-w-0">
                                    ...
                                    <input type="datetime-local" value={form.playback_from} onChange={(event) => onUpdateForm('playback_from', snapTo10Min(event.target.value))} className="w-full min-w-0 rounded-lg border border-edge-strong px-2 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
                                </label>
                                <label className="block w-full min-w-0">
                                    <span className="mb-1 block text-xs text-content-subtle">Sampai</span>
                                    <input type="datetime-local" value={form.playback_to} onChange={(event) => onUpdateForm('playback_to', snapTo10Min(event.target.value))} className="w-full min-w-0 rounded-lg border border-edge-strong px-2 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
                                </label>
                            </div>
                        ) : (
                            <div className="flex gap-2">
                                <input type="number" min="1" value={form.playback_window_value} onChange={(event) => onUpdateForm('playback_window_value', event.target.value)} placeholder="Kosong = semua" className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
                    ...
                    <label className="block min-w-0">
                        ...
                        <input type="datetime-local" value={form.expires_at} onChange={(event) => onUpdateForm('expires_at', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-4">
                <label className="block min-w-0">
                    ...
                    <select value={form.access_code_mode} onChange={(event) => onUpdateForm('access_code_mode', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white">
                ...
                <label className="block min-w-0">
                     ...
                     */}
                    <input value={form.access_code_mode === 'custom' ? form.custom_access_code : form.access_code_length} maxLength={form.access_code_mode === 'custom' ? 32 : undefined} onChange={(event) => onUpdateForm(form.access_code_mode === 'custom' ? 'custom_access_code' : 'access_code_length', form.access_code_mode === 'custom' ? event.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, '') : event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
                </label>
                <label className="block min-w-0">
                    <span className="mb-1 block text-sm font-medium text-content-muted">Limit Device</span>
                    <input type="number" min="0" value={form.max_active_sessions} onChange={(event) => onUpdateForm('max_active_sessions', event.target.value)} placeholder="Preset" className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
                </label>
                <label className="block min-w-0">
                    <span className="mb-1 block text-sm font-medium text-content-muted">Mode Limit</span>
                    <select value={form.session_limit_mode} onChange={(event) => onUpdateForm('session_limit_mode', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white">
                                    ...
                                    <div className="mt-2">
                                        ...
                                        <div className="grid gap-2 sm:grid-cols-4">
                                            <label className="block min-w-0">
                                                ...
                                                <input type="number" min="1" placeholder="ikut token" value={form.camera_rules[camera.id]?.playback_window_hours || ''} onChange={(event) => onUpdateCameraRule(camera.id, 'playback_window_hours', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-2 py-1 text-base sm:text-xs dark:bg-gray-950 dark:text-white" />
                                            </label>
                                            <label className="block min-w-0">
                                                <span className="mb-0.5 block text-[10px] text-content-subtle">Berlaku sampai</span>
                                                <input type="datetime-local" value={form.camera_rules[camera.id]?.expires_at || ''} onChange={(event) => onUpdateCameraRule(camera.id, 'expires_at', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-2 py-1 text-base sm:text-xs dark:bg-gray-950 dark:text-white" />
                                            </label>
                                            <label className="block min-w-0">
                                                ...
                                                <select
                                                    ...
                                                    className="w-full min-w-0 rounded-lg border border-edge-strong bg-surface px-2 py-1 text-base sm:text-xs text-content"
                                                >
                                            ...
                                            <label className="block min-w-0">
                                                ...
                                                <input placeholder="—" value={form.camera_rules[camera.id]?.note || ''} onChange={(event) => onUpdateCameraRule(camera.id, 'note', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-2 py-1 text-base sm:text-xs dark:bg-gray-950 dark:text-white" />
                                            </label>
                                        </div>
                                    </div>
                                )}
            ...
            <label className="mt-4 block">
                ...
                <textarea rows={5} value={form.share_template} onChange={(event) => onUpdateForm('share_template', event.target.value)} className="w-full min-w-0 rounded-lg border border-edge-strong px-3 py-2 text-base sm:text-sm dark:bg-gray-950 dark:text-white" />
            </label>
