...
export default function AreaCard({
    ...
    return (
        <div className="bg-surface border border-edge rounded-[26px] p-5 hover:shadow-xl hover:border-primary/30 transition-all group shadow-sm">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex flex-wrap justify-end gap-1.5">
                    <button type="button" title="Pengaturan Massal Kamera" onClick={() => onOpenBulkConfig(area)} className="p-2 rounded-lg bg-surface-sunken text-content-muted hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-500/10 transition-all">
                    ...
                    <button type="button" title="Hapus Semua Kamera" onClick={() => onBulkDelete(area)} className="p-2 rounded-lg bg-surface-sunken text-content-muted hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <h3 className="text-[1.7rem] leading-tight font-bold text-content mb-2">{area.name}</h3>
            <p className="text-sm text-content-muted flex items-center gap-2 mb-3">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a2 2 0 01-2.828 0l-4.243-4.243a8 8 0 1111.314 0z" />
                </svg>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                ...
                {gridDefaultEnabled ? (
                    ...
                    <span className="text-[10px] font-semibold px-2 py-1 rounded-full bg-surface-sunken text-content-muted">
                        Grid Default Off
                    </span>
                )}
            ...
            <div className="grid grid-cols-3 gap-2 mb-3 text-xs">
                <div className="rounded-xl border border-edge bg-surface-sunken px-3 py-2">
                    <div className="text-content-muted">Kamera</div>
                    <div className="font-semibold text-content">{area.cameraCount || 0}</div>
                </div>
                <div className="rounded-xl border border-edge bg-surface-sunken px-3 py-2">
                    <div className="text-content-muted">Online</div>
                    <div className="font-semibold text-emerald-700 dark:text-emerald-300">{area.onlineCount || 0}</div>
                </div>
                <div className="rounded-xl border border-edge bg-surface-sunken px-3 py-2">
                    <div className="text-content-muted">Offline</div>
                    <div className="font-semibold text-red-700 dark:text-red-300">{area.offlineCount || 0}</div>
                </div>
            </div>
            <div className="rounded-2xl border border-edge bg-surface-sunken px-4 py-3 mb-4">
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                    ...
                    <div className="text-right font-semibold text-content">{getAreaCoverageLabel(area.coverage_scope)}</div>
                    <div className="text-content-muted">Focus Zoom</div>
                    ...
                    <div className="text-content-muted">Grid Default</div>
                    <div className="text-right font-semibold text-content">{gridDefaultEnabled ? 'Enabled' : 'Hidden'}</div>
                    <div className="text-content-muted">Limit Grid</div>
                    <div className="text-right font-semibold text-content">{area.grid_default_camera_limit ? `${area.grid_default_camera_limit} kamera` : 'Tanpa batas'}</div>
                    <div className="text-content-muted">Internal RTSP Policy</div>
                    <div className="text-right font-semibold text-content">{internalIngestLabel}</div>
                    <div className="text-content-muted">Idle Close</div>
                    <div className="text-right font-semibold text-content">{area.internal_on_demand_close_after_seconds ? `${area.internal_on_demand_close_after_seconds} detik` : 'Ikuti default'}</div>
                </div>
            </div>
            <div className="mb-4 grid gap-3">
                ...
                <div className="rounded-2xl border border-edge bg-surface-sunken px-4 py-3">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <div className="text-sm font-semibold text-content">Limit Kamera Grid Default</div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-edge">
                <span className="text-sm font-medium text-content-muted">{area.internalValidCount || 0} Internal &bull; {area.externalValidCount || 0} External</span>
