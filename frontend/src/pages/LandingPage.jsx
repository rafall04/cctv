...
// ============================================
const VideoPlayer = memo(function VideoPlayer({ camera, compact = false, autoPlay = true }) {
    ...
    // Intersection Observer (only if not autoPlay)
    useEffect(() => {
        if (autoPlay) return;
...
// ============================================
// CUSTOM 3-CAMERA LAYOUT - Play simultaneously
// ============================================
function ThreeCameraLayout({ cameras, onClose }) {
    const containerRef = useRef(null);
    ...
    return (
        <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col">
            ...
            <div className="flex items-center justify-between p-3 sm:p-4 border-b border-white/10 bg-gray-900/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-xs">{cameras.length} cameras</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={toggleFS} className="p-2 hover:bg-white/10 rounded-xl text-white">
                        <Icons.Fullscreen />
                    </button>
            ...
            {/* Video Grid */}
            <div ref={containerRef} className="flex-1 p-2 sm:p-4 overflow-hidden">
                {cameras.length === 1 && (
                    <div className="h-full">
                        <div className="h-full rounded-xl overflow-hidden">
                            <VideoPlayer camera={cameras[0]} autoPlay={true} />
                        </div>
                    </div>
                )}
                ...
                {cameras.length === 2 && (
                    <div className="h-full grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
                        {cameras.map(cam => (
                            <div key={cam.id} className="rounded-xl overflow-hidden">
                                <VideoPlayer camera={cam} compact autoPlay={true} />
                            </div>
                        ))}
                    </div>
                )}
...
export default function LandingPage() {
    ...
    return (
        <div className={`min-h-screen flex flex-col transition-colors ${dark ? 'bg-gray-950' : 'bg-gray-50'}`}>
            ...
            {/* Simple Header */}
            <div className={`py-6 sm:py-10 text-center px-4 ${dark ? 'bg-gray-900/50' : 'bg-white'}`}>
                ...
                <p className={`text-sm sm:text-base ${dark ? 'text-gray-400' : 'text-gray-600'}`}>
                    Klik kamera untuk melihat stream. Gunakan tombol + untuk multi-view (max 3).
                </p>
            </div>
