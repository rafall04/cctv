...
- **DB dual-connection — hazard mitigated, full convergence deferred:** `database/database.js` now sets `busy_timeout=5000` (matching connectionPool), removing the SQLITE_BUSY asymmetry that was the concrete hazard. The 15 modules below still use its own single connection rather than `connectionPool` — full convergence is **deferred**, and is NOT a mechanical import swap: connectionPool's separate read/write connections break the read-after-write consistency those modules rely on. Needs a per-module audit. Modules: `adminDashboardService`, `apiKeyService`, `backupService`, `brandingService`, `bruteForceProtection`, `feedbackService`, `notificationDiagnosticsService`, `passwordExpiry`, `passwordHistory`, `saweriaService`, `settingsService`, `telegramService`, `thumbnailService`, `timezoneService`, `userService`.
  - ⚠️ **`cameraRuntimeStateService` already uses `connectionPool`** (it is not one of the 15) **and the read-after-write hazard is real there, not theoretical.** `cameraHealthService` upserts runtime state for every camera from inside a single `transaction()`; within that transaction the read connection *cannot* see a row the write connection just INSERTed, so for a camera with no runtime row yet the read-back returns nothing — a certainty, not a race. Production logged 22 × `[CameraHealth] Check failed: Cannot read properties of undefined (reading 'last_runtime_signal_at')`, and because the upserts share one transaction and the try/catch wraps the whole tick, one such camera rolled back every camera's state and left the rest unchecked. `ensureRuntimeState` now returns the values it just wrote instead of the failed read-back (contract: never undefined when the table exists), and the monitoring stage isolates per-camera failures. Regression tests: `__tests__/cameraRuntimeStateService.test.js`. **Any module moved onto `connectionPool` needs this same read-back audit.**
