...
- Playback public/admin: `/playback` uses `accessScope='public_preview'`; `/admin/playback` uses `accessScope='admin_full'`; `Playback.jsx` -> `recordingService` -> recording routes -> segment stream endpoint. The picker list comes from `GET /api/cameras/playback` (optional auth) -> `cameraService.getPlaybackCameraList(user)`: a slim recording-cameras-only projection (picker fields + `video_codec`, no runtime/billing/source URLs) — admin scope returns every `enable_recording=1` camera, public scope only `enabled=1 AND camera_class='community'` (the stricter archive rule), with userinfo stripped from `external_snapshot_url`.
- Admin dashboard payload split: `useDashboardData` polls `GET /api/admin/stats?streams=0` every 10s (summary/system/breakdown/topCameras/recentLogs/allSessions — no stream table) and loads the full stream table separately via `GET /api/admin/stats/streams` on mount, every 30s, and on each "all streams" drawer open; both reads share `adminDashboardService.loadStreamContext()` so the split can't drift.
- Archive streaming (Telegram): segments whose local file was pruned are served by `telegramArchiveLibraryService.openSegmentStream` — disk-first, then MTProto sidecar `GET /segment/:id` (Range-capable), then Bot API `getFile` fallback. After the first MTProto slice is served, a background prefetch pulls the whole segment **via Bot API getFile** (hardlink into `rafnet-arch-seg-<id>.mp4` in the managed transit dir `archiveCacheService.CACHE_DIR`) — never via the sidecar, whose single Pyrogram client would starve live slices behind a full download (measured: seek → 504). Subsequent seeks/plays read local disk; each MTProto slice otherwise costs a fresh Telegram round-trip (~8-28s on a saturated box).
...
- ⚠️ **Focused runs skip the guardrails**: `npm test -- <file>` filters them out, so a ratchet/doc-lint breach only surfaces in the full suite (or CI). While iterating on a frozen file, add `guardrails` to the filter. The ratchet counts every line **including comments** — the frozen giants (e.g. `MapView.jsx`) sit near their ceilings, so shrink changes there rather than raising the baseline.
...
- ✅ **External URL credential exposure — RESOLVED 2026-08-30 (52b001fe):** `stripUrlCredentials` removes `user:pass@` from every `external_*_url` in anonymous/public payloads (`streamService.js` emitter path). The URL *host* is still emitted to anonymous clients — a privacy / proxy-bypass consideration, not a credential leak — but credentials no longer are.
...
- ✅ ~~Area mutations lack audit logging~~ — RESOLVED: `areaController.js` logs create/update/delete via `logAdminAction` (`:93` / `:120` / `:164`), matching camera mutations.
- ✅ ~~Prod env template does not set `API_KEY_REQUIRE_KEYS=true`~~ — RESOLVED 2026-09-19. The flag now ships in `deployment/backend.env.prod` + both `generate-env*.sh` scripts. The prerequisite that made it unsafe is fixed in the same pass: `apiKeyValidator.js`'s whitelist now covers the whole anonymous surface (auth csrf/refresh, viewer runtime-signal, playback-viewer, playback-token/archive, thumbnails, public settings reads, sponsors, saweria, branding, config bootstrap, voucher, feedback, recordings, promo/affiliate media), `POST /api/admin/api-keys` is exempt as the bootstrap path (still `authMiddleware + requireAdmin`), and CORS `OPTIONS` preflights are skipped. The full anonymous/protected classification is pinned in `__tests__/apiKeyValidator.test.js` so future whitelist rot fails loudly.
...
- ✅ ~~`backend/services/backupService.js:111` uses `INSERT OR REPLACE`~~ — **RESOLVED.** The restore path no longer uses `INSERT OR REPLACE` (which silently deletes a conflicting row). It now uses explicit conflict handling — `INSERT OR IGNORE` / `ON CONFLICT(pk) DO NOTHING` (merge mode) or `DO UPDATE SET` (overwrite mode) — and a row it cannot allow is **refused and reported** so an operator decides, not destroyed (`backupService.js:108`, `171-182`).
