...
if [ -f "$APP_DIR/backend/data/INITIAL_ADMIN_PASSWORD.txt" ]; then
    ...
    echo "   (juga tersimpan di $APP_DIR/backend/data/INITIAL_ADMIN_PASSWORD.txt — hapus setelah login)"
else
...
echo ""
echo "📝 Next Steps:"
echo "   1. Login, GANTI password admin, lalu hapus INITIAL_ADMIN_PASSWORD.txt"
echo "   2. Setup SSL: sudo certbot --nginx"
echo "   3. (Opsional, pengerasan) Generate API Key dari panel admin → frontend/.env → rebuild."
echo "      Aplikasi sudah berjalan penuh tanpa langkah ini."
echo "   4. Cek kesehatan: bash $APP_DIR/deployment/healthcheck.sh"
echo ""
