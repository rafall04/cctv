...
server {
    ...
    # robots.txt + sitemap.xml are generated by the backend so the Sitemap line and the
    # /area/<slug> entries track THIS deployment's live domain/areas — static files would
    # need per-domain editing and silently rot as areas are added. X-Forwarded-Proto must
    # pass the ORIGINAL client scheme (Cloudflare's header), not the internal hop's $scheme.
    location = /robots.txt {
    ...
    location / {
        ...
        add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' https://static.cloudflareinsights.com https://www.topcreativeformat.com https://*.topcreativeformat.com https://*.effectivegatecpm.com https://inklinkor.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https: http:; font-src 'self' data:; connect-src 'self' ws: wss: https://nominatim.openstreetmap.org https://cctvjss.jogjakota.go.id https://cctvkanjeng.gresikkab.go.id https://surakarta.atcsindonesia.info:8086 https://*.effectivegatecpm.com https://*.topcreativeformat.com https://inklinkor.com; media-src 'self' blob: http: https:; worker-src 'self' blob:; manifest-src 'self'; frame-src https: http:; form-action 'self'; base-uri 'none'; object-src 'none'; frame-ancestors 'none'" always;
