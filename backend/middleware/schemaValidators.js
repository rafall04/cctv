...
// ============================================
// Monetag Settings Schema
// ============================================

export const monetagSettingsSchema = {
    body: {
        type: 'object',
        properties: {
            popunder_enabled: {
...
// Validator middleware for Monetag settings
export async function validateMonetagSettings(request, reply) {
    try {
        await request.compileValidationSchema(monetagSettingsSchema.body)(request.body);
    } catch (error) {
        if (error.validation) {
            return reply.code(400).send({
                success: false,
                message: 'Invalid Monetag settings data',
                errors: error.validation.map(v => ({
                    field: v.instancePath?.replace('/', '') || v.params?.missingProperty || 'body',
                    message: v.message
                }))
